...
def get_recipients_for_config(config_name: str) -> list[str]:
    """
    Get email addresses of active subscribers for a specific config.

    Args:
        config_name: The config key
    ...
    try:
        ...
        emails = list(
            Subscriber.objects.filter(
                is_active=True,
                configs__key=config_name,
            ).values_list("email", flat=True),
        )

        logger.info(
            f"Found {len(emails)} recipients for {config_name}: "
            f"{', '.join(emails)}",
        )

    except Exception:
        logger.exception(
            f"Error querying database for config {config_name}.",
        )
        return []
    else:
        return emails
...
def get_available_newsletters() -> list[str]:
    """
    Get display names of all available newsletters from the database.

    Returns:
        List of newsletter display names sorted alphabetically

    """
    try:

        return list(
            NewsConfig.objects.order_by("display_name").values_list(
                "display_name",
                flat=True,
            ),
        )
...
def execute(report_path: Path, analysis_data: AnalysisData) -> None:
    ...
    # Get the config name from analysis_data
    ...
    topic = analysis_data.get("config_name", "News")
    ...
    if email_receivers_override is not None:
        ...
        # Get recipients from Django database based on config_name
        recipient_emails = get_recipients_for_config(config_name)

        if not recipient_emails:
            logger.warning(
                f"No active recipients found for config: {config_name}",
            )
            return
    ...
    email_body = replace_placeholders_in_report(
        report_html,
        sender_email,
    )
