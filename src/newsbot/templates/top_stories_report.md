...
**Cross-Source News Coverage & Sentiment Analysis**
...
{% endif %}
{% if source in analysis.source_summaries %}
{% for item in analysis.source_summaries[source] %}
**[{{ item.article.title }}]({{ item.article.url }})**{% if item.article.sentiment %} [{{ item.article.sentiment.label }}]{% endif %}  
...
*Generated by NewsBot on {{ date }}*  
*This report analyzes how different news sources cover the same stories*
